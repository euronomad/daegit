...
--parameters publicip_name=mongodkIP03 \
--parameters dnsPrefix=mongodk03 \
--parameters nic_name=mongodkNic03
...
--parameters vm_name=mongoTestVM03 \
...
--parameters dataDisk_name=mongodbDataDisk31
az group deployment create --name createDataDisk --resource-group daekimMongoDBRG01 --template-file 01.dataDisk.json \
--parameters dataDisk_name=mongodbDataDisk32

# data disk attach
az vm disk attach --vm-name mongoTestVM03 --disk mongodbDataDisk31 --lun 0 --resource-group daekimMongoDBRG01
az vm disk attach --vm-name mongoTestVM03 --disk mongodbDataDisk32 --lun 1 --resource-group daekimMongoDBRG01

#disk lun 확인
az vm show --resource-group daekimMongoDBRG01 --name mongoTestVM03 | jq -r .storageProfile.dataDisks
az vm show --resource-group daekimMongoDBRG01 --name mongoTestVM03 | jq -r .storageProfile.dataDisks[].lun